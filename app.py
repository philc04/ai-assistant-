...
- Keep very long chat responses inside their message bubble on mobile/desktop.
...
# IMPORTANT:
# This file must be saved as atlas_v010.py NEXT TO the existing app.py.
# It is a patch layer, not a replacement for app.py.
if __name__ == "app":
    raise RuntimeError(
        "Save this file as atlas_v010.py next to app.py. "
        "Do not replace app.py with this patch file."
    )

import app as base


EXPECTED_BASE_VERSION = "0.9.1-alpha"
PATCH_VERSION = "0.10.0-alpha"

if getattr(base, "APP_VERSION", None) != EXPECTED_BASE_VERSION:
    raise RuntimeError(
        f"atlas_v010.py was checked against Atlas {EXPECTED_BASE_VERSION}, "
        f"but app.py reports {getattr(base, 'APP_VERSION', 'unknown')}. "
        "Review the patch against the current base before deploying."
    )
...
CHAT_CSS = r"""
/* Keep long URLs, code-like strings and giant model responses inside the chat bubble. */
.msg{
    max-width:100%;
