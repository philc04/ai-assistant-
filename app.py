...
async def chat(req: ChatRequest):
    ...
    instructions = (
        ...
        "Treat memories marked as user corrections as strong instructions for similar future requests. "
    ...
    with db() as conn:
        with conn.cursor() as cur:
            ...
            if req.correction and req.correction.strip():
                ...
                prior = cur.fetchone()
                if prior:
                    cur.execute("INSERT INTO training_candidates(source_type, input_text, target_text) VALUES ('user_correction', %s, %s)", (prior["content"], correction))
                    memory_text = f"User correction for request '{prior['content']}': {correction}"
                    cur.execute("INSERT INTO memories(kind, content, importance, confidence, source) VALUES ('correction', %s, 1.0, 1.0, 'user_correction')", (memory_text,))
                    candidate = True
...
def home():
    return HTMLResponse(r'''<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#0d0d0f"><title>Atlas</title><style>body{margin:0;background:#0d0d0f;color:#b78cff;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.w{max-width:760px;margin:auto;padding:18px;min-height:100vh}.h{display:flex;justify-content:space-between;align-items:center}.brand{font-size:26px;font-weight:800;color:#c39bff}.muted{color:#8f6ccf;font-size:12px}.card{background:#151518;border:1px solid #303036;border-radius:16px;padding:14px;margin:12px 0}.m{padding:12px;border-radius:14px;margin:10px 0;white-space:pre-wrap;line-height:1.45;color:#c6a6ff}.u{background:#29292f;margin-left:12%}.a{background:#17171b;border:1px solid #303038;margin-right:7%}textarea,input{width:100%;box-sizing:border-box;background:#0f0f12;color:#d8c5ff;border:1px solid #3a3a42;border-radius:12px;padding:12px;font-size:16px}textarea::placeholder,input::placeholder{color:#715b99}textarea{min-height:58px}button{border:1px solid #3a3a42;border-radius:11px;padding:11px 13px;font-weight:700;background:#242429;color:#c6a6ff}.row{display:flex;gap:8px}.row button{flex:1;background:#29292f}.opts{display:flex;gap:16px;padding:10px 0;color:#a987dc;font-size:14px}.opts input{width:auto}.small{font-size:12px;color:#9a7bc4}.fb{display:flex;gap:6px;margin-top:8px}.fb button{padding:7px 9px;background:#222227;color:#bda0ec}.fb button:disabled{opacity:.7}#login{display:none}</style></head><body><div class=w><div class=h><div><div class=brand>Atlas</div><div class=muted>OpenAI now. Atlas Core stays ours.</div></div><button onclick="toggleLogin()">Access</button></div><div id=login class=card><input id=k type=password placeholder="Private Atlas access key"><button style="margin-top:8px" onclick="saveKey()">Save on this phone</button></div><div id=c></div><div class=card><textarea id=p placeholder="Ask Atlas anything..."></textarea><div class=opts><label><input id=web type=checkbox> Web search</label><label><input id=deep type=checkbox> Deep model</label></div><div class=row><button onclick="send()">Send</button><button onclick="teach()">Teach Atlas</button></div><div id=s class=small style="margin-top:8px"></div></div></div><script>let cid=localStorage.getItem('atlas_cid')||null,key=localStorage.getItem('atlas_key')||'';k.value=key;if(!key)login.style.display='block';function toggleLogin(){login.style.display=login.style.display==='block'?'none':'block'}function saveKey(){key=k.value.trim();localStorage.setItem('atlas_key',key);login.style.display='none';s.textContent='Access key saved on this phone.'}function add(t,who,id){let d=document.createElement('div');d.className='m '+(who==='u'?'u':'a');d.append(document.createTextNode(t));if(who==='a'&&id){let f=document.createElement('div');f.className='fb';let good=document.createElement('button');good.textContent='Useful';good.onclick=async()=>{if(await rate(id,1,null)){good.textContent='Saved â';good.disabled=true}};let fix=document.createElement('button');fix.textContent='Needs fix';fix.onclick=()=>correct(id,fix);f.append(good,fix);d.appendChild(f)}c.appendChild(d);window.scrollTo(0,document.body.scrollHeight)}async function send(){let m=p.value.trim();if(!m)return;if(!key){toggleLogin();return}add(m,'u');p.value='';s.textContent='Atlas is working...';try{let r=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json','X-Atlas-Key':key},body:JSON.stringify({message:m,conversation_id:cid,use_web:web.checked,deep:deep.checked})});let j=await r.json();if(!r.ok)throw new Error(j.detail||'Failed');cid=j.conversation_id;localStorage.setItem('atlas_cid',cid);add(j.answer,'a',j.assistant_message_id);s.textContent='Model: '+j.model}catch(e){add('Error: '+e.message,'a');s.textContent=''}}async function teach(){if(!key){toggleLogin();return}let x=prompt('What should Atlas remember?');if(!x)return;try{let r=await fetch('/api/memory',{method:'POST',headers:{'Content-Type':'application/json','X-Atlas-Key':key},body:JSON.stringify({content:x})});let j=await r.json();if(!r.ok)throw new Error(j.detail||'Could not save it.');s.textContent='Saved to Atlas memory.'}catch(e){s.textContent='Memory error: '+e.message}}async function rate(id,rating,correction){try{let r=await fetch('/api/feedback',{method:'POST',headers:{'Content-Type':'application/json','X-Atlas-Key':key},body:JSON.stringify({message_id:id,rating,correction})});let j=await r.json();if(!r.ok)throw new Error(j.detail||'Could not save feedback.');s.textContent=correction?'Correction saved to Atlas memory.':'Feedback saved.';return true}catch(e){s.textContent='Feedback error: '+e.message;return false}}async function correct(id,button){let x=prompt('What should Atlas have answered or done instead?');if(!x)return;let ok=await rate(id,-1,x);if(ok){button.textContent='Correction saved â';button.disabled=true}}p.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}})</script></body></html>''')
